      ...
      <section className="rounded border border-slate-800 p-4">
        ...
        <form action={createNews} className="grid gap-3">
          <label className="flex flex-col gap-1">
            ...
            <input name="title" required className="bg-transparent border border-slate-700 rounded px-2 py-1" />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-sm opacity-80">Contenido</span>
            <textarea name="content" rows={5} required className="bg-transparent border border-slate-700 rounded px-2 py-1" />
          </label>
          <div>
            <button className="border border-slate-700 rounded px-3 py-1">Publicar</button>
          </div>
        </form>
      </section>
      <ul className="space-y-3">
  {(news as NewsItem[] | null ?? []).map((n) => (
          <li key={n.id} className="border border-slate-800 rounded p-3 space-y-2">
            <form action={updateNews} className="grid gap-2">
              <input type="hidden" name="id" value={n.id} />
              <label className="flex flex-col gap-1">
                <span className="text-xs opacity-70">Título</span>
                <input name="title" defaultValue={n.title} className="bg-transparent border border-slate-700 rounded px-2 py-1" />
              </label>
              <label className="flex flex-col gap-1">
                <span className="text-xs opacity-70">Contenido</span>
                <textarea name="content" defaultValue={n.content} rows={4} className="bg-transparent border border-slate-700 rounded px-2 py-1" />
              </label>
              <div><button className="border border-slate-700 rounded px-3 py-1">Guardar</button></div>
            </form>
